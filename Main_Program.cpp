...
int main() {
    ...
    /*
    for (int i = 0; i < 256; i++) {
        std::cout << histogram1[i] << ",";   //打印第一个灰度矩阵特征值的统计直方图
    }
    */  
    freopen("person7-1.txt", "r", stdin);//源文件
    ...
    /*
    for (int i = 0; i < 256; i++) {
        std::cout << histogram2[i] << ",";   //打印第二个灰度矩阵特征值的统计直方图
    }
    */
    HistogramNormalization(histogram1, histogram2);
    //std::cout<< std::endl;
    std::cout<< "chi测度为：" <<( chi=chiSquaredDistance(histogram1, histogram2) )<< std::endl;
